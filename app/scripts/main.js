$(document).ready(function(){
	...
	var orderButton = function(elem) {elem.innerHTML='Заказать товар и уточнить наличие на складе и прочие подробности можно по телефонам: <b><br>+375 (29) 641-46-13<br>+375 (232) 75-80-87</b>';
	...
	var print = function () {
		$('#results').append('<div id="u'+i+'" class="position"><img class="preview" src='+json_a[value].img+' alt="'+json_a[value].name+'" /><h5>'
			+json_a[value].name + '</h5><div class="modal-market"><div><span class="close">X</span>'+'<img src='+json_a[value].img+' alt="'+json_a[value].name+'" /><h6>'
      +json_a[value].name+'</h6><p>'
			+json_a[value].descr +'</p><a href="" id="button" class="waves-effect waves-light btn" onclick="orderButton(this);">Заказать</a></div></div></div>');
	};

	$.getJSON('../mylist.json', {}, function(json){  // загрузка JSON данных из файла .json
		var	json_a=json; });
	var i=1, j=1, j_flag=1;
	clickable();
	var  id_tag='', id_brand='';
	$(".sort li").click(function (e) {
		...
		if ((id_brand!='')&&(id_tag=='')) {
			$('#results').animate({opacity: 'toggle'},200, function () {
				$('#results').html('');
				for (value in json_a) {
					if (id_brand==json_a[value].brand)
						print();
						i++;
				}
				i=1;
